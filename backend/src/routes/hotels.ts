...
router.get('/search', async(req:Request, res:Response) => {
    try {
        ...
        let sortOptios = {};
        switch(req.query.sortOptios){
            case 'starRating':
                sortOptios = {sortOptios: -1};
            break;
            case 'pricePerNightAsc':
                sortOptios = {pricePerNight: 1};
            break;
        ...
        const hotels = await Hotel.find(query).sort(sortOptios).skip(skip).limit(pageSize);
